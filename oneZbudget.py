...
def getbudgetvars(fil1,fil2,fil3,fil4,dt):
    cond1 = getvar(fil3,'reflectivity')
    cond2 = getvar(fil4,'reflectivity')
    rho = (getvar(fil1, 'press') * 100.) / (getvar(fil1, 'tempk') *287.)
    w = getvar(fil1, 'w')
    u = getvar(fil1,'u')
    v = getvar(fil1,'v')
    micro2 = np.zeros_like(w)#= (getvar(fil2, 'nuccldrt') + getvar(fil2, 'nucicert') + getvar(fil2, 'vapliqt') + getvar(fil2, 'vapicet')) / 1000.
    pcp3d =np.zeros_like(w) #= (getvar(fil1,'precip3d')/ rho)
...
def makeplots(fil1,fil2,fil3,fil4,dt):
    w,diff,horiz,vert,term,micro,res = getbudgetvars(fil1,fil2,fil3,fil4,dt)
    colmax = np.max(w,0)
...
#    plt.colorbar()
   ...
   # plt.colorbar()
    ...
    cb1.update_ticks()
    plt.savefig('budget10km-ref.png')
    plt.clf()
    plt.close()
...
#    plt.xlim(-1e-6,1e-6)
    plt.legend()
    plt.savefig('budget-profsw1-ref.png')
    plt.clf()



files = sorted(glob.glob('/nobackup/rstorer/convperts/mature/feb23-control/*h5'))
radarfiles = sorted(glob.glob('/nobackup/rstorer/convperts/mature/quickbeam/feb23-control*h5'))

outdir = '/nobackup/rstorer/plots/'

num= 8
makeplots(files[num],files[num+3],radarfiles[num],radarfiles[num+3],90.)
