...
for case in cases:
  ...
  for xdir in modeldirs:
      ...
      for i in range(numfiles):
          ...
          slicevar = np.squeeze(fil['w'].value[:,:,160:170,:])
          print slicevar.shape
          pcp[xdir].append(np.mean(slicevar,1))
          print pcp[xdir][i].shape
...
#  levels=np.logspace(-2,1.3,20)
  ...
  for i in range(numfiles):
      ...
      if height.max() < 50:
      	plt.ylim(0,18)
      cbar = plt.colorbar(f)
      plt.suptitle('Vertical Velocity '+str(i*30) + ' s', size = 20)
      plt.savefig('../plots/wslice'+case+outi+'.png')
      plt.close()

  os.system("ffmpeg -framerate 10 -pattern_type glob -i '../plots/wslice"+case+"*.png' -c:v libx264 -pix_fmt yuv420p ../plots/wslice"+case+".mp4") 
