...
def getbudgetvars(fil1,fil2,dt):
    ...
    pcp3d = (getvar(fil1,'precip3d')/ rho)
    ...
    dzdz[1:-1,:,:] = (cond1[2:,:,:]-cond1[:-2,:,:]) / (z[2:,None,None]-z[:-2,None,None])

    dzdx = np.zeros_like(u)
    dzdx[:,:,1:-1] = (cond1[:,:,2:]-cond1[:,:,:-2])/(500.)

    dzdy = np.zeros_like(v)
    dzdy[:,1:-1,:] = (cond1[:,2:,:]-cond1[:,:-2,:])/(500.)

    term = np.zeros_like(w)
    term[1:-1,:,:] = (pcp3d[2:,:,:]-pcp3d[:-2,:,:]) / (z[2:,None,None]-z[:-2,None,None])

    vert = -1*w*dzdz
    horiz = (u*dzdx + v*dzdy) * -1

    rhs = mic + horiz + vert + term
    res = diff-rhs

    return dzdz, cond1, w,diff,horiz,vert,term,mic,res

def makeplots(fil1,fil2,dt):
    dzdz, cond, w,diff,horiz,vert,term,micro,res = getbudgetvars(fil1,fil2,dt)
    colmax = np.max(w,0)
    w1 = np.where(colmax>1)
    ys = np.arange(400)*.25
    levs = np.linspace(-1.2,1.2,151)*1e-4
    print levs
...
#    ext = np.mean(res[:,w1[0],w1[1]],1) 
    ...
    wdiag = (diff + horiz - micro - term) / (dzdz)
    wdiag[np.isnan(wdiag)]=0.

    wtrue=np.zeros_like(height)
    west = np.zeros_like(height)
    ...
    for ix in range(2,80):
        ww =np.squeeze(w[ix,:,:])
        cond1 = np.squeeze(cond[ix,:,:])
        ww1 = np.where(np.logical_and(ww>1,cond1>0.0001))
        ...
        wtrue[ix]=np.mean(w[ix,ww1[0],ww1[1]])
        west[ix]=np.mean(wdiag[ix,ww1[0],ww1[1]])
...
#    plt.xlim(-1e-6,1e-6)
    plt.legend()
    plt.savefig('budget-profsw1.png')
    plt.clf()


    plt.plot(wtrue[2:],height[2:],color = 'black', linewidth=3,label = 'W True')
#    plt.plot(west[2:],height[2:],color = 'gray', linewidth=3, label = 'W Diag')
    plt.legend()
    plt.savefig('budget-wdiag.png')
    plt.clf()
...
num= 8
makeplots(files[num],files[num+3],90.)
